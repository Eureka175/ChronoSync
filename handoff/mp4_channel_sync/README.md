...
| **容器 edit list / 逐流头部裁剪**（MP4 常见） | 部分 ffmpeg 版本在解码或重封装多流 PCM 时会裁掉个别流的头部样本（实测 305 样本）——延迟会被系统性偏移"被裁掉的样本数"，而四流 start_time 仍报 0，**没有任何显式错误** | ① 解码时强制 `-ignore_editlist 1`；② **务必校验各流解码样本数是否相等**（同一录制的四流必然等长），不等长即视为容器异常、该片交付前必须复核；③ 不要在没有该校验的情况下信任"小延迟"结论 |
