...
**v0.1.0-beta** · MIT · Python 3.12+ · [English](README.md)
...
+ JSON/CSV 导出 + CLI `batch` 多轨批处理 + **MP4 无线麦通道同步专线**。
...
| Synthetic Framework（seed 化，15 场景） | ✅ `synthetic/` |
...
| MP4 无线麦通道同步（测量→修正→重封装→复检） | ✅ `src/chronosync/mp4sync/` |
| 单元/集成测试（**202 项 pytest + 16 项交付包自测**） | ✅ `tests/`、`handoff/` |
| Benchmark（gcc/features/drift/solve） | ✅ `benchmarks/` |
| CLI（info / gcc / align / batch / **mp4-sync** / test-gcc / benchmark / --json） | ✅ `chronosync` |
| 外部工程交付包（仅 numpy+scipy） | ✅ `handoff/mp4_channel_sync/` |
| Reaper RPP 导出 / 大规模真实录音基准 | ⏳ Phase 8-9 |
...
pytest、psutil（dev/benchmark）。FFmpeg（任意较新版本）仅 MP4 专线与相关
集成测试需要。
...
| `docs/mp4_wireless_delay_case.md` | 真实素材无线麦延迟案例 |
| `docs/adr/` | ADR 001–012 |
| `handoff/mp4_channel_sync/README.md` | 外部工程交付包（纯算法 + 集成指南） |
...
MIT（见 `LICENSE`）。第三方库各自遵循其许可证（numpy/scipy BSD；
SoXR 与 libsndfile 为 LGPL，动态链接并保留声明）。
